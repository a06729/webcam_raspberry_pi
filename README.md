...
## 문제 해결: "연결 활성화가 실패했습니다" (Connection activation failed)

BLE 로 와이파이 정보를 보냈는데 활성화 단계에서 실패하는 경우.

**일시적 실패 (가장 흔함)** — 라즈베리파이의 와이파이와 블루투스는 **안테나를
공유**해서, 노트북과 BLE 가 연결된 상태(프로비저닝 중)나 부팅 직후에는 첫
활성화가 간헐적으로 실패한다. 코드가 자동으로 **재스캔 후 최대 3회 재시도**
하므로 대부분 두 번째 시도 안에 붙는다. 로그에서 확인:

```
활성화 실패 (1/3회): ...      ← 일시 실패
와이파이 연결 성공: 'MyWifi' (2회째 시도)   ← 재시도로 성공
```

**3회 모두 실패하면** 회신되는 메시지로 원인을 구분한다:

| 실패 메시지 | 원인 / 조치 |
|---|---|
| `Secrets were required …` (비밀번호 힌트 표시) | 비밀번호 오타 — 노트북에서 다시 전송 |
| SSID 를 찾지 못함 | SSID 오타, 또는 5GHz 전용 AP (파이 모델에 따라 2.4GHz 만 지원) |
| `IP configuration could not be reserved` | 공유기 DHCP 문제 — 공유기 재시작, MAC 필터 확인 |
| 반복되는 원인 불명 실패 | `journalctl -u NetworkManager -b` 로 상세 사유 확인 |
